...
return $.ajax({dataType:"json",url:"http://pipes.yahoo.com/pipes/pipe.run",data:f,jsonp:"_callback"}).always(function(h){$.each(h.value.items,function(i,j){if(j.songs!==null){app.playlist=$.merge(app.playlist,j.songs);
e=$.merge(e,j.songs);}});});}$("#playlist").on("artistsLoaded",function(h,g){var f=[];var d=[];$.each(g,function(i,e){d.push(c(e,f));});$.when.apply($,d).then(function(e){$("#playlist").trigger("playlistUpdated",[f]);
});}).on("playlistUpdated",function(g,f){f=shuffleArray(f);var d=this;$.each(f,function(e,i){var h=$('<a href="#" class="list-group-item">'+i.artist.name+" - "+i.name+"</a>");
h.data("song",i);$(d).append(h);});$("#loading").removeClass("in");currentIndex++;$("#playlist a:eq("+currentIndex+")").trigger("click");}).on("click","a",function(g){g.preventDefault();
var d=$(this);$("#playlist a").removeClass("active");d.addClass("active");currentIndex=$(this).index();curSong=d.data("song");var h=[];var f=curSong.artist.name+" - "+curSong.name;
$("title").html(f);VK.Api.call("audio.search",{q:f,sort:2,count:10},function(j){if(j.error){if(j.error.error_code===7){VK.Auth.logout(function(){$("#vk-login").prop("disabled",false);
